    ...
    >
      <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '0.4em 0.5em' }}>
              <p className="font-data" style={{
                fontSize: 'clamp(7px, 1.2cqi, 14px)', color: 'var(--gs-light)', fontWeight: 600,
              ...
              {displayDesc && (
                <p className="font-data" style={{
                  fontSize: 'clamp(7px, 1.1cqi, 13px)', color: 'var(--gs-mid)', lineHeight: 1.3,
                  display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden',
                }}>
                  {displayDesc}
                </p>
              )}
              <p className="font-data" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)', opacity: 0.5, marginTop: '0.15em' }}>
                {scan.domain}
              </p>
            </div>
          </div>
            ...
            <div style={{ marginTop: '0.4em', display: 'flex', flexDirection: 'column', gap: '0.15em' }}>
              {profileLinks.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3em', flexWrap: 'wrap' }}>
                  <span className="font-data" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)' }}>Profiles:</span>
                  <span className="font-data" style={{ fontSize: 'clamp(7px, 1.1cqi, 13px)', color: 'var(--gs-light)' }}>
                    {profileLinks.join(', ')}
                  </span>
                </div>
              )}
              {shareButtons.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3em', flexWrap: 'wrap' }}>
                  <span className="font-data" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)' }}>Share:</span>
                  <span className="font-data" style={{ fontSize: 'clamp(7px, 1.1cqi, 13px)', color: 'var(--gs-light)' }}>
                    {shareButtons.join(', ')}
                  </span>
                </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5em' }}>
          {/* OG Tags */}
          <div>
            <p className="font-data uppercase" style={{ fontSize: 'clamp(7px, 1.1cqi, 13px)', color: 'var(--gs-base)', letterSpacing: '0.1em', marginBottom: '0.2em' }}>
              Open Graph
            </p>
