...
export function M37Slide({ scan }: { scan: ScanWithResults }) {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '3%' }}>
          ...
          <div style={{ flex: '0 0 35%', display: 'flex', flexDirection: 'column', gap: '0.5em' }}>
            ...
            {avgRating != null && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5em', marginBottom: '0.15em' }}>
                  <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 3cqi, 36px)', fontWeight: 700, color: 'var(--gs-light)', lineHeight: 1 }}>
                    {avgRating.toFixed(1)}
                  </span>
            ...
            {trend && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3em', marginTop: '0.2em' }}>
                <span style={{
                  fontSize: 'clamp(1px, 1.5cqi, 18px)',
                ...
                <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', textTransform: 'capitalize' }}>
                  {trend} trend
                </span>
              </div>
            )}
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {monthlyBuckets.length === 0 && (
              <p className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-mid)', opacity: 0.6 }}>
