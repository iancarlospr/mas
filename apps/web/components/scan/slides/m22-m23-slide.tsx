...
export function M22M23Slide({ scan }: { scan: ScanWithResults }) {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '3%' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {/* Notable mention */}
            {newsNotable && (
              <p className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', marginBottom: '0.3em', fontStyle: 'italic', opacity: 0.8 }}>
                {newsNotable}
              </p>
            )}
            ...
            {!hasNewsData && newsHeadlines.length === 0 && (
              <p className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-mid)', opacity: 0.5, padding: '0.5em 0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2em' }}>
                {newsHeadlines.map((h, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: sentimentColor(h.sentiment), flexShrink: 0, marginTop: '0.1em' }}>
                      {h.sentiment === 'positive' ? '+' : h.sentiment === 'negative' ? '-' : '\u00B7'}
                    </span>
                    <div>
                      <p className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', lineHeight: 1.3 }}>
                        {h.title ?? 'Untitled'}
                      </p>
                      {h.source && (
                        <p className="font-data" style={{ fontSize: 'clamp(1px, 1cqi, 12px)', color: 'var(--gs-mid)', opacity: 0.6 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2em' }}>
                {socialMentions.map((m, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: sentimentColor(m.sentiment), flexShrink: 0, marginTop: '0.1em' }}>
