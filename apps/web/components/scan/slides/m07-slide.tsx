...
export function M07Slide({ scan, chloeCallout }: { scan: ScanWithResults; chloeCallout?: React.ReactNode }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5em', overflow: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3em' }}>
            {Array.from(categoryMap.entries()).map(([cat, names], ci) => (
              <div key={cat} style={{ display: 'flex', alignItems: 'baseline', gap: '0.5em' }}>
                <span className="font-data" style={{ fontSize: 'clamp(1px, 0.83cqi, 13px)', color: catColors[ci % catColors.length], fontWeight: 600, minWidth: '8em', flexShrink: 0 }}>
                  {cat}
                </span>
      ...
      {missingCategories.length > 0 && (
        <div style={{ paddingTop: '0.3em', borderTop: '1px solid rgba(255,178,239,0.06)' }}>
          <p className="font-data uppercase" style={{ fontSize: 'clamp(1px, 0.83cqi, 13px)', color: 'var(--gs-critical)', letterSpacing: '0.08em', marginBottom: '0.2em', opacity: 0.8 }}>
            Missing Categories
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.2em' }}>
      ...
      {forms.length > 0 && (
        <div style={{ paddingTop: '0.3em', borderTop: '1px solid rgba(255,178,239,0.06)' }}>
          <p className="font-data uppercase" style={{ fontSize: 'clamp(1px, 0.83cqi, 13px)', color: 'var(--gs-mid)', letterSpacing: '0.08em', marginBottom: '0.2em' }}>
            Forms Detected
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.1em' }}>
            {forms.map((f, i) => (
              <p key={i} className="font-data" style={{ fontSize: 'clamp(1px, 0.86cqi, 13px)', color: 'var(--gs-light)', opacity: 0.8 }}>
