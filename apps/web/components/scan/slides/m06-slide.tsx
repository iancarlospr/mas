      ...
      {pixels.length > 0 ? (
        <div style={{ overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div className="font-data uppercase" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)', padding: '0.2em 0.3em', letterSpacing: '0.05em' }}>
              Pixel
            </div>
            <div className="font-data uppercase" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)', padding: '0.2em', textAlign: 'center', letterSpacing: '0.05em' }}>
              Enhanced
            </div>
            <div className="font-data uppercase" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)', padding: '0.2em', textAlign: 'center', letterSpacing: '0.05em' }}>
              Server
          ...
          {pixels.slice(0, 8).map((px, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <div className="font-data" style={{ fontSize: 'clamp(7px, 1.15cqi, 14px)', color: 'var(--gs-light)', padding: '0.25em 0.3em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {pixelName}
                </div>
                <div style={{ textAlign: 'center', padding: '0.25em' }}>
                  <span className="font-display" style={{ fontSize: 'clamp(7px, 1.3cqi, 15px)', fontWeight: 700, color: enhanced.color }}>{enhanced.sym}</span>
                </div>
                <div style={{ textAlign: 'center', padding: '0.25em' }}>
                  <span className="font-display" style={{ fontSize: 'clamp(7px, 1.3cqi, 15px)', fontWeight: 700, color: server.color }}>{server.sym}</span>
                </div>
                <div style={{ textAlign: 'center', padding: '0.25em' }}>
                  <span className="font-display" style={{ fontSize: 'clamp(7px, 1.3cqi, 15px)', fontWeight: 700, color: consent.color }}>{consent.sym}</span>
                </div>
                <div className="font-data tabular-nums" style={{ fontSize: 'clamp(7px, 1.15cqi, 14px)', color: 'var(--gs-light)', textAlign: 'center', padding: '0.25em', fontWeight: 600 }}>
                  {fires}
                </div>
              </div>
            );
