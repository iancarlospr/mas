...
export function M34Slide({ scan, chloeCallout }: { scan: ScanWithResults; chloeCallout?: React.ReactNode }) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4em', marginBottom: '0.2em' }}>
        ...
        <span className="font-data" style={{
          fontSize: 'clamp(1px, 0.86cqi, 13px)', color: 'var(--gs-light)', flex: 1,
        ...
        <span className="font-data tabular-nums" style={{
          fontSize: 'clamp(1px, 0.86cqi, 13px)', fontWeight: 600, color: arrowColor,
          flexShrink: 0,
        }}>
          #{kw.rankAbsolute}
        </span>
        ...
        {kw.searchVolume > 0 && (
          <span className="font-data tabular-nums" style={{
            fontSize: 'clamp(1px, 0.75cqi, 12px)', color: 'var(--gs-mid)', flexShrink: 0,
          }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4em', height: '100%' }}>
        ...
        {[
          ...
          <div key={i} style={{
            ...
            <p className="font-data tabular-nums" style={{
              fontSize: 'clamp(1px, 1.50cqi, 24px)', fontWeight: 700, lineHeight: 1, color: s.color,
            }}>
              {s.value}
            </p>
      ...
      <div style={{ display: 'flex', flex: 1, gap: '3%', minHeight: 0 }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <p className="font-data uppercase" style={{
            fontSize: 'clamp(1px, 0.83cqi, 13px)', letterSpacing: '0.1em', marginBottom: '0.25em',
          ...
          {topGaining.length > 0 ? (
            ...
            <p className="font-data" style={{ fontSize: 'clamp(1px, 0.83cqi, 13px)', color: 'var(--gs-mid)', opacity: 0.6 }}>
      ...
      {(gainingCount + losingCount) > 0 && (
        <div style={{ marginTop: '0.2em' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.1em' }}>
            <span className="font-data" style={{ fontSize: 'clamp(1px, 0.75cqi, 12px)', color: 'var(--gs-mid)' }}>Stability ratio</span>
            <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 0.75cqi, 12px)', color: netColor }}>
