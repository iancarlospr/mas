      ...
      <div style={{ display: 'flex', gap: '0.6em', alignItems: 'stretch' }}>
        <div style={{
          ...
          <p className="font-data tabular-nums" style={{
            fontSize: 'clamp(28px, 3.5cqi, 42px)', fontWeight: 700, lineHeight: 1, color: 'var(--gs-light)',
          }}>
          ...
          {merchantName && (
            <p className="font-data" style={{
              fontSize: 'clamp(12px, 1cqi, 12px)', color: 'var(--gs-mid)', marginTop: '0.2em',
            }}>
              {merchantName}
            </p>
          )}
            ...
            {products.map((p, i) => (
              <div key={i} style={{
                ...
                <span className="font-data tabular-nums" style={{
                  fontSize: 'clamp(12px, 1.1cqi, 13px)', color: 'var(--gs-mid)',
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <p className="font-data" style={{
                    fontSize: 'clamp(12px, 1.2cqi, 14px)', color: 'var(--gs-light)', lineHeight: 1.3,
                    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                  }}>
                    {p.title}
                  </p>
                  {p.seller && (
                    <p className="font-data" style={{ fontSize: 'clamp(12px, 1cqi, 12px)', color: 'var(--gs-mid)', lineHeight: 1.2 }}>
                      {p.seller}
                    </p>
                  )}
                </div>
                {p.price > 0 && (
                  <span className="font-data tabular-nums" style={{
                    fontSize: 'clamp(12px, 1.2cqi, 14px)', fontWeight: 600, color: 'var(--gs-light)',
                    flexShrink: 0,
                  }}>
