  ...
  return (
    <div style={{ marginBottom: '0.15em' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.15em' }}>
        <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-mid)' }}>{label}</span>
        <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color, fontWeight: 600 }}>{value} chars</span>
      </div>
...
function TagDots({ tags, label }: { tags: Array<{ key: string; present: boolean }>; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4em', marginBottom: '0.2em' }}>
      <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', minWidth: '6em' }}>{label}</span>
      <div style={{ display: 'flex', gap: '0.3em', alignItems: 'center', flexWrap: 'wrap' }}>
        {tags.map((t) => (
          <div key={t.key} style={{ display: 'flex', alignItems: 'center', gap: '0.15em' }}>
            ...
            <span className="font-data" style={{ fontSize: 'clamp(1px, 1cqi, 12px)', color: t.present ? 'var(--gs-light)' : 'var(--gs-mid)', opacity: t.present ? 0.8 : 0.4 }}>
              {t.key}
            </span>
          </div>
        ))}
...
export function M04Slide({ scan }: { scan: ScanWithResults }) {
  ...
  const vizContent = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35em' }}>
      ...
      {titleContent && (
        <div>
          <p className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', opacity: 0.7, marginBottom: '0.1em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
      ...
      {hasJsonLd && (
        <div style={{ display: 'flex', gap: '0.3em', flexWrap: 'wrap', alignItems: 'center', marginTop: '0.1em' }}>
          <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-mid)' }}>JSON-LD:</span>
      ...
      {hreflangArr.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3em', marginTop: '0.1em' }}>
          <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-mid)' }}>Hreflang:</span>
          <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.15cqi, 14px)', color: 'var(--gs-light)', fontWeight: 600 }}>
