...
export function M39Slide({ scan }: { scan: ScanWithResults }) {
              ...
              <div style={{ marginBottom: '0.4em' }}>
                ...
                {freshness && (
                  <div style={{ display: 'flex', gap: '1.5em', marginTop: '0.2em' }}>
                    <span className="font-data tabular-nums" style={{ fontSize: 'clamp(12px, 1.1cqi, 13px)', color: 'var(--gs-light)' }}>
                      {freshness.updatedLast30Days} updated &lt;30d
                    </span>
                    <span className="font-data tabular-nums" style={{ fontSize: 'clamp(12px, 1.1cqi, 13px)', color: 'var(--gs-mid)' }}>
          ...
          <div style={{ flex: 1 }}>
                ...
                {sitemaps.map((sm, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4em', marginBottom: '0.1em' }}>
                      <span style={{
                        fontSize: 'clamp(13px, 1.3cqi, 15px)', fontWeight: 700,
                        color: sm.valid ? 'var(--gs-terminal)' : 'var(--gs-critical)',
                      }}>
                        {sm.valid ? '\u2713' : '\u2717'}
                      </span>
                      <span className="font-data" style={{
                        fontSize: 'clamp(12px, 1.15cqi, 13px)', color: 'var(--gs-light)',
                        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1,
                      }}>
                        {shortUrl}
                      </span>
                      {sm.valid && (
                        <span className="font-data tabular-nums" style={{ fontSize: 'clamp(12px, 1.1cqi, 13px)', color: 'var(--gs-mid)', flexShrink: 0 }}>
            ...
            {locales.length > 0 && (
              <div style={{ marginTop: '0.2em' }}>
                <span className="font-data" style={{ fontSize: 'clamp(12px, 1.1cqi, 13px)', color: 'var(--gs-mid)' }}>
