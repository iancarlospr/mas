      ...
      >
        <div className="relative z-10 h-full flex flex-col overflow-hidden" style={{ padding: '1.5% 3.5% 1%' }}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center" style={{ gap: '1em' }}>
            <span className="font-display" style={{ fontSize: 'clamp(1px, 3.75cqi, 60px)', opacity: 0.15, color: 'var(--gs-base)' }}>
              M40
            </span>
          ...
          <div style={{
            ...
            {stats.map((s) => (
              <div key={s.label} style={{
                ...
                <p className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.65cqi, 26px)', fontWeight: 700, color: s.color, lineHeight: 1 }}>
                  {s.value}
                </p>
          ...
          {(() => {
            ...
            return (
              <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                {showChat ? (
                  <ChloeCallout question={chatQ} onAskChloe={onAskChloe!} recommendations={recs} scanId={scan.id} slideId="M40" />
                ) : (
                  <>
                    <h4 className="font-display uppercase" style={{ fontSize: 'clamp(1px, 0.98cqi, 14px)', letterSpacing: '0.18em', color: 'var(--gs-base)', marginBottom: '0.4em' }}>
                      Recommendations
