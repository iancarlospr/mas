  ...
  const vizContent = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4em' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.15em' }}>
        <h4 className="font-display uppercase" style={{ fontSize: 'clamp(7px, 1.2cqi, 13px)', letterSpacing: '0.18em', color: 'var(--gs-base)' }}>
          Behavioral Maturity Model
        </h4>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3em' }}>
          <span className="font-data tabular-nums" style={{ fontSize: 'clamp(10px, 2.2cqi, 26px)', fontWeight: 700, color: maturityColor, lineHeight: 1 }}>
            {detectedCount}/{capabilities.length}
          </span>
          <span className="font-data" style={{ fontSize: 'clamp(7px, 1.1cqi, 13px)', color: maturityColor }}>
            {maturityLabel}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3em', marginTop: '0.3em' }}>
        {experiments.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.2em', alignItems: 'center' }}>
            <span className="font-data uppercase" style={{ fontSize: 'clamp(7px, 1cqi, 12px)', color: 'var(--gs-mid)', marginRight: '0.3em', letterSpacing: '0.05em' }}>
              A/B:
            </span>
      ...
      {overlays.length > 0 && (
        <div style={{ marginTop: '0.15em' }}>
          <span className="font-data" style={{ fontSize: 'clamp(7px, 1.1cqi, 13px)', color: 'var(--gs-warning)' }}>
