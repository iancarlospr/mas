    ...
    >
      <div className="relative z-10 h-full flex flex-col overflow-hidden" style={{ padding: '1.5% 3.5% 1%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.5em', flexShrink: 0 }}>
          ...
          {score != null && (
            <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.5cqi, 17px)', fontWeight: 700, color: scoreColor(score) }}>
        ...
        <h2 className="font-display" style={{
          fontSize: 'clamp(1px, 2.8cqi, 32px)', fontWeight: 700, lineHeight: 1.15,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3em' }}>
                {scoreBreakdown.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5em' }}>
                    ...
                    <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.35cqi, 15px)', fontWeight: 600, color: scoreColor(s.score), minWidth: '2em', textAlign: 'right' }}>
                      {s.score}
                    </span>
                  </div>
                ))}
...
export function StatBlock({ value, label, color = 'var(--gs-light)' }: { value: string | number; label: string; color?: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <p className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 2.5cqi, 30px)', fontWeight: 700, color, lineHeight: 1 }}>
        {value}
      </p>
      <p className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-mid)', marginTop: '0.2em' }}>
        {label}
      </p>
    </div>
  );
...
export function SvgGauge({ value, max = 100, label, thresholds }: {
  ...
  return (
    <div style={{ textAlign: 'center', flex: '0 0 auto' }}>
      ...
      <p className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 2cqi, 22px)', fontWeight: 700, color, marginTop: '-0.6em', lineHeight: 1 }}>
        {typeof value === 'number' ? (value % 1 !== 0 ? value.toFixed(value < 1 ? 3 : 1) : value) : value}
      </p>
      <p className="font-data" style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: 'var(--gs-mid)', marginTop: '0.15em' }}>
        {label}
      </p>
    </div>
  );
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '1em', marginTop: '0.3em' }}>
        {segments.filter(s => s.value > 0).map((s, i) => (
          <span key={i} className="font-data" style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: s.color }}>
...
export function MatrixGrid({ rows, columns }: {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `auto repeat(${columns.length}, 1fr)`, gap: '1px' }}>
      <div />
      {columns.map(c => (
        <div key={c} className="font-data uppercase" style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: 'var(--gs-base)', textAlign: 'center', padding: '0.2em', letterSpacing: '0.05em' }}>
          {c}
        </div>
      ))}
      {rows.map(r => (
        <React.Fragment key={r.label}>
          <div className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-light)', padding: '0.2em 0.4em 0.2em 0', whiteSpace: 'nowrap' }}>
            {r.label}
          </div>
          {r.cells.map((c, ci) => {
            ...
            return (
              <div key={`${r.label}-${ci}`} style={{ background: s.bg, textAlign: 'center', padding: '0.2em', borderRadius: '2px' }}>
                <span className="font-display" style={{ fontSize: 'clamp(1px, 1.4cqi, 16px)', color: s.color, fontWeight: 700 }}>
                  {s.sym}
                </span>
              </div>
            );
...
export function StackLayer({ label, value, confidence }: { label: string; value: string; confidence?: number }) {
  return (
    <div style={{
      ...
      <span className="font-data uppercase" style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color: 'var(--gs-mid)', minWidth: '6em', letterSpacing: '0.05em' }}>
        {label}
      </span>
      <span className="font-data" style={{ fontSize: 'clamp(1px, 1.3cqi, 15px)', color: 'var(--gs-light)', fontWeight: 600 }}>
        {value}
      </span>
      {confidence != null && (
        <span className="font-data" style={{ fontSize: 'clamp(1px, 1cqi, 12px)', color: 'var(--gs-mid)', marginLeft: 'auto' }}>
...
export function FreshnessBar({ daysAgo, maxDays = 365 }: { daysAgo: number; maxDays?: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5em' }}>
      ...
      <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.1cqi, 13px)', color, whiteSpace: 'nowrap' }}>
        {label}
      </span>
    </div>
  );
...
export function CheckItem({ status, label, detail }: { status: 'pass' | 'fail' | 'warn'; label: string; detail?: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.4em' }}>
      <span className="font-display" style={{ fontSize: 'clamp(1px, 1.5cqi, 17px)', fontWeight: 700, color: st.color, lineHeight: 1, flexShrink: 0, marginTop: '0.05em' }}>
        {st.sym}
      </span>
      <div>
        <p className="font-data" style={{ fontSize: 'clamp(1px, 1.25cqi, 14px)', color: 'var(--gs-light)', lineHeight: 1.3 }}>
          {label}
        </p>
        {detail && (
          <p className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-mid)', lineHeight: 1.3 }}>
            {detail}
          </p>
        )}
      </div>
...
export function RankedBar({ label, value, max, color = 'var(--gs-base)', suffix = '' }: { label: string; value: number; max: number; color?: string; suffix?: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5em' }}>
      <span className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-light)', minWidth: '30%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {label}
      </span>
...
export function Pill({ text, color = 'var(--gs-light)' }: { text: string; color?: string }) {
  return (
    <span className="font-data" style={{
      fontSize: 'clamp(1px, 1cqi, 13px)', color, padding: '0.15em 0.5em', borderRadius: '3px',
  ...
  return (
    ...
    >
      <div className="relative z-10 h-full flex flex-col overflow-hidden" style={{ padding: '1.5% 3.5% 1%' }}>
        ...
        {/* Headline */}
        <h2 className="font-display" style={{
          fontSize: 'clamp(1px, 2.4cqi, 28px)', fontWeight: 700, lineHeight: 1.15,
        ...
        {/* Exec summary */}
        {execSummary && (
          <p className="font-data" style={{ fontSize: 'clamp(1px, 1.25cqi, 14px)', lineHeight: 1.5, color: 'var(--gs-light)', opacity: 0.85, marginBottom: '0.5em', flexShrink: 0 }}>
            {execSummary}
          </p>
        )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25em' }}>
                  {findings.map((f, i) => (
                    <div key={i} style={{ display: 'flex', gap: '0.3em', alignItems: 'flex-start' }}>
                      <span className="font-data uppercase flex-shrink-0" style={{
                        fontSize: 'clamp(1px, 1.15cqi, 13px)', padding: '0.05em 0.25em', borderRadius: '2px',
                        background: `color-mix(in srgb, ${severityColor(f.severity)} 15%, transparent)`,
                        color: severityColor(f.severity), marginTop: '0.1em', fontWeight: 600,
                      }}>
                        {severityLabel(f.severity)}
                      </span>
                      <p className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-light)', lineHeight: 1.35 }}>
                        {f.finding}
                      </p>
                    </div>
                  ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2em' }}>
                  {scoreBreakdown.map((s, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4em' }}>
                      <span className="font-data" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', color: 'var(--gs-light)', flex: 1 }}>{s.criterion}</span>
                      <div style={{ width: '36px', height: '3px', background: 'rgba(255,255,255,0.06)', borderRadius: '2px', overflow: 'hidden', flexShrink: 0 }}>
                        <div style={{ width: `${s.score}%`, height: '100%', background: scoreColor(s.score), borderRadius: '2px' }} />
                      </div>
                      <span className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.15cqi, 13px)', fontWeight: 600, color: scoreColor(s.score), minWidth: '1.5em', textAlign: 'right' }}>{s.score}</span>
                    </div>
                  ))}
                </div>
              </div>
...
export function SkippedSlide({ moduleName, scan, sourceLabel }: { moduleName: string; scan: ScanWithResults; sourceLabel: string }) {
  return (
    <div className="slide-card relative overflow-hidden select-none"
      ...
      <div className="relative z-10 h-full flex flex-col overflow-hidden" style={{ padding: '1.5% 3.5% 1%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.5em', flexShrink: 0 }}>
          <span className="font-display uppercase" style={{ fontSize: 'clamp(1px, 1.4cqi, 15px)', letterSpacing: '0.18em', color: 'var(--gs-base)' }}>{moduleName}</span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center" style={{ gap: '1em' }}>
          <p className="font-data" style={{ fontSize: 'clamp(1px, 1.6cqi, 18px)', color: 'var(--gs-mid)', textAlign: 'center', maxWidth: '70%' }}>
            This module was not available for this scan.
          </p>
          <p className="font-data" style={{ fontSize: 'clamp(1px, 1.3cqi, 15px)', color: 'var(--gs-mid)', opacity: 0.6, textAlign: 'center' }}>
            Re-run the scan to generate this analysis.
          </p>
        </div>
        <div style={{ padding: '0.6em 0 0', display: 'flex', justifyContent: 'space-between', flexShrink: 0 }}>
          <span className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-mid)', opacity: 0.4 }}>{sourceLabel}</span>
          <span className="font-data" style={{ fontSize: 'clamp(1px, 1.2cqi, 14px)', color: 'var(--gs-mid)', opacity: 0.4 }}>{scan.domain} — AlphaScan</span>
        </div>
      </div>
    </div>
  );
