...
export function M08Slide({ scan, chloeCallout }: { scan: ScanWithResults; chloeCallout?: React.ReactNode }) {
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${(blockingScripts > 0 || asyncScripts > 0) ? 3 : 2}, 1fr)`, gap: '0.5em' }}>
          ...
          <div style={{
            ...
            <p className="font-data" style={{ fontSize: 'clamp(1px, 0.98cqi, 16px)', fontWeight: 700, lineHeight: 1.2, color: tmsArr.length > 0 ? 'var(--gs-light)' : 'var(--gs-critical)' }}>
            ...
            {tmsCount > 1 && (
              <p className="font-data" style={{ fontSize: 'clamp(1px, 0.68cqi, 12px)', color: 'var(--gs-critical)', marginTop: '0.1em' }}>
          ...
          <div style={{
            ...
            <p className="font-display" style={{ fontSize: 'clamp(1px, 1.80cqi, 28px)', fontWeight: 700, lineHeight: 1, color: serverSideIndicators ? 'var(--gs-terminal)' : 'var(--gs-critical)' }}>
            ...
            {sstSources.length > 0 && (
              <p className="font-data" style={{ fontSize: 'clamp(1px, 0.68cqi, 12px)', color: 'var(--gs-mid)', marginTop: '0.1em' }}>
                {sstSources[0]}
              </p>
            )}
          </div>
          ...
          {(blockingScripts > 0 || asyncScripts > 0) && (
            <div style={{
              ...
              <p className="font-data tabular-nums" style={{ fontSize: 'clamp(1px, 1.50cqi, 24px)', fontWeight: 700, lineHeight: 1, color: blockingScripts > 5 ? 'var(--gs-critical)' : blockingScripts > 0 ? 'var(--gs-warning)' : 'var(--gs-terminal)' }}>
                {blockingScripts}
              </p>
              <p className="font-data uppercase" style={{ fontSize: 'clamp(1px, 0.68cqi, 12px)', color: 'var(--gs-base)', letterSpacing: '0.06em', marginTop: '0.25em' }}>
                Render-Blocking
