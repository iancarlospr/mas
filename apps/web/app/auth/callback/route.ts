 ...
 *  to ensure auth cookies persist on the redirect response. */
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    console.error('[auth/callback] No code parameter in OAuth callback');
    return NextResponse.redirect(new URL('/auth/error?message=invalid_link', origin));
  }

  const redirectUrl = new URL(redirectTo, origin);
  const isScanGate = redirectUrl.searchParams.get('scan_gate') === 'true';
  ...
  // Send welcome email only for new users (created within last 60s)
  ...
  // The original tab's AuthGatePoller detects verification by polling signInWithPassword()
  // every 3s — works cross-browser and cross-device.
  if (isScanGate) {
