...
export async function POST(
  ...
  if (!rl.allowed) {
    getPostHog()?.capture({ distinctId: user.id, event: 'rate_limit_hit', properties: { endpoint: 'unlock' } });
  ...
  // PostHog: scan unlocked with credits (server-side)
  getPostHog()?.capture({
    distinctId: user.id,
    event: 'scan_unlocked_with_credits',
    properties: { scan_id: scanId, credits_remaining: creditsRemaining },
  });
