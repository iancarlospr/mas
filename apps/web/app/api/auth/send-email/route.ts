...
export async function POST(request: NextRequest) {
  ...
  try {
    switch (email_data.email_action_type) {
      case 'signup': {
        const confirmUrl = `${baseUrl}/auth/confirm?token_hash=${email_data.token_hash}&type=signup&redirect_to=/history`;
        await sendEmail({
          to: user.email,
          userId: user.id,
      ...
      case 'magiclink': {
        const magicUrl = `${baseUrl}/auth/confirm?token_hash=${email_data.token_hash}&type=magiclink&redirect_to=/history`;
        await sendEmail({
          to: user.email,
          userId: user.id,
          template: 'magic-link',
