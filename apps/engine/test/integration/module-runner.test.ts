...
describe('ModuleRunner Integration', () => {
  ...
  it('should run all phases for full tier', async () => {
    const runner = new ModuleRunner('test-scan-1', 'https://test.example.com', 'paid');
  ...
  it('should run all phases for full tier', async () => {
    const runner = new ModuleRunner('test-scan-2', 'https://test.example.com', 'paid');
  ...
  it('should update scan status for each phase', async () => {
    const runner = new ModuleRunner('test-scan-3', 'https://test.example.com', 'paid');
    await runner.run();
  ...
  it('should upsert each module result to Supabase', async () => {
    const runner = new ModuleRunner('test-scan-4', 'https://test.example.com', 'paid');
    await runner.run();
  ...
  it('should make previousResults available to downstream modules', async () => {
    ...
    const runner = new ModuleRunner('test-scan-5', 'https://test.example.com', 'paid');
    await runner.run();
