...
vi.mock('../../src/ghostscan/browser-pool.js', () => {
  const mockPage = {
    ...
    content: vi.fn().mockResolvedValue('<html><head><title>Works</title></head><body><h1>Real Content</h1><p>Paragraph with enough text to pass the 100 character sanity check in the browser HTML retry path</p></body></html>'),
...
vi.mock('../../src/modules/registry.js', () => ({
  getModulesForPhaseAndTier: vi.fn().mockImplementation((phase: string) => {
    if (phase === 'passive') return [
      { id: 'M04', name: 'Page Metadata', phase: 'passive', minimumTier: 'paid', timeout: 30000, retries: 0, category: 'seo_content' },
    ];
    if (phase === 'browser') return [
      { id: 'M03', name: 'Performance', phase: 'browser', minimumTier: 'paid', timeout: 60000, retries: 0, category: 'performance_ux' },
    ];
    return [];
  }),
...
describe('Passive Module Retry', () => {
  ...
  it('should retry failed passive modules with browser HTML', async () => {
    const runner = new ModuleRunner('retry-test', 'https://etsy-like.example.com', 'paid');
