...
const PAD_X = 56;            // ≥ 3σ of wide blur so glow fades to <5% at edge
const PAD_Y = 44;
...
async function main() {
  ...
  // Wide ambient glow — σ=18, fades to ~5% at PAD_Y (44px ≈ 2.4σ)
  const glow1 = await sharp(upscaled)
    .extend(pad)
    .blur(18)
    .toBuffer();

  // Tight glow — σ=9, fades to ~0% well within padding
